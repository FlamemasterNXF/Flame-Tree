...
function addNegativePoints(layer, gain) {
	player[layer].points = player[layer].points.add(gain).min(0)
	if (player[layer].best) player[layer].best = player[layer].best.min(player[layer].points)
	if (player[layer].total) player[layer].total = player[layer].total.add(gain)
}

function generatePoints(layer, diff) {
	if (diff < 0) addNegativePoints(layer, tmp[layer].resetGain.times(diff))
	else addPoints(layer, tmp[layer].resetGain.times(diff))
}
