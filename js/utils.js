...
function canAffordUpgrade(layer, id) {
	...
	if (cost !== undefined) {
		if (cost >= 0){ return canAffordPurchase(layer, upg, cost) }
		else { return  canAffordPurchaseNegative(layer, upg, cost) }
	}
...
function canAffordPurchaseNegative(layer, thing, cost) {
	if (thing.currencyInternalName) {
		let name = thing.currencyInternalName
		if (thing.currencyLocation) {
			return !(thing.currencyLocation[name].gt(cost))
		}
		else if (thing.currencyLayer) {
			let lr = thing.currencyLayer
			return !(player[lr][name].gt(cost))
		}
		else {
			return !(player[name].gt(cost))
		}
	}
	else {
		return !(player[layer].points.gt(cost))
	}
}
	...
	if (pay !== undefined)
		...
		if (upg.currencyInternalName) {
			...
			if (upg.currencyLocation) {
				if (upg.currencyLocation[name].gte(0)){ if (upg.currencyLocation[name].lt(cost)) return }
				if (upg.currencyLocation[name].lt(0)){ if (upg.currencyLocation[name].gt(cost)) return }
				upg.currencyLocation[name] = upg.currencyLocation[name].sub(cost)
			}
			else if (upg.currencyLayer) {
				let lr = upg.currencyLayer
				if (player[lr][name].gte(0)){ if (player[lr][name].lt(cost)) return }
				if (player[lr][name].lt(0)){ if (player[lr][name].gt(cost)) return }
				player[lr][name] = player[lr][name].sub(cost)
			}
			else {
				if (player[lr][name].gte(0)){ if (player[name].lt(cost)) return }
				if (player[lr][name].lt(0)){ if (player[name].gt(cost)) return }
				player[name] = player[name].sub(cost)
			}
		}
		else {
			if (player[layer].points.gte(0)){ if (player[layer].points.lt(cost)) return }
			if (player[layer].points.lt(0)){ if (player[layer].points.gt(cost)) return }
